...
uint8_t i2c0_write_n_byte(uint8_t DevAddr, uint8_t RegAddr, uint8_t *buf, uint8_t nBytes)
{
    ...
    DL_I2C_fillControllerTXFIFO(I2C_MPU6050_INST, &buf[0], nBytes);
    ...
    DL_I2C_fillControllerTXFIFO(I2C_MPU6050_INST, &temp_reg_dddr, 1);
    ...
    n = 0;
    do {
        Byte4Fill = DL_I2C_getControllerTXFIFOCounter(I2C_MPU6050_INST);
        if(Byte4Fill > 1)
        {
            DL_I2C_fillControllerTXFIFO(I2C_MPU6050_INST, &buf[n], 1);
            n++;
        }
        }
    while(n<nBytes);
